...
def _llm_json_guard_fallback() -> str:
    return json.dumps({
        "is_phishing": None,
...
def _call_gemini_json(prompt_text: str) -> str:
...
def generate_rag_answer(user_query: str, k: int = 5) -> str:
    ...
    if not ctxs:
        return _llm_json_guard_fallback()
...
def transcribe_audio_with_gemini(audio_bytes: bytes, mime_type: str = "audio/wav") -> str:
    ...
    parts = [
        types.Part.from_text(
            "다음 음성을 한국어 텍스트로 정확히 전사해줘. "
            "타임스탬프/스피커 표기/주석 없이 원문만 반환해."
        ),
...
def generate_rag_answer_from_audio(
    ...
    try:
        ...
        if not transcript:
            return "", _llm_json_guard_fallback()
        rag_json = generate_rag_answer(transcript, k=k)
        return transcript, rag_json
    except Exception:
        return "", _llm_json_guard_fallback()
